...
const EditInventoryItemForm: React.FC<EditInventoryItemFormProps> = ({
  ...
  const [uploadedImage, setUploadedImage] = useState<string>(item.image || "");
  const [user.role, setUserRole] = useState<string>("Administrator");
  const { toast } = useToast();
  
  // Get the user user.role from session storage
  useEffect(() => {
    const userInfo = sessionStorage.getItem("user");
    if (userInfo) {
      try {
        const userData = JSON.parse(userInfo);
        setUserRole(userData.user.role || "Administrator");
      } catch (error) {
  ...
  const handleBarcodeScan = (result: string) => {
    ...
    toast({
      ...
      description: `Barcode ${result} has been successfully scanned`,
    });
  };
  ...
  const onSubmit = async (data: InventoryItemFormValues) => {
    ...
    try {
      // Include image and barcode data but filter out price fields for stockers
      let formData: any = {
        ...data,
        image: uploadedImage,
      ...
      // Remove price fields if user is a Stocker
      if (user.role === "Stocker") {
        // Create a new object without price-related properties
        // Stocker users cannot modify price information
        const { price, priceUnit, ...filteredData } = formData;
        ...
        <div className="mb-6">
          ...
          {form.watch("barcode") && (
            <div className="p-2 bg-blue-50 text-blue-700 rounded-md mb-4">
              <p className="text-sm">Barcode: <span className="font-medium">{form.watch("barcode")}</span></p>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label htmlFor="price" className="block text-sm font-medium text-gray-700 mb-1">
              Price
              {user.role === "Stocker" && (
                <span className="ml-1 text-xs text-amber-600">(read-only for Stocker accounts)</span>
              )}
            </label>
            {user.role === "Stocker" ? (
              <input
