import React, { useRef, useEffect, useState } from './../../node_modules/.vite/deps_temp_343513f7/react';
import { useReactToPrint } from 'react-to-print';
import { useQuery } from '@tanstack/react-query';
import { openCashDrawer } from "./src/utils/peripheralDetection";
...
const PrintReceipt: React.FC<PrintReceiptProps> = ({
  ...
  // Attempt to open cash drawer when receipt is shown
  useEffect(() => {
    if (isOpen) {
      ...
      openCashDrawer().then(success => {
        if (success) {
          ...
          toast({
            title: "Cash drawer operation",
            description: "No cash drawer detected or failed to open",
            variant: "destructive"
          });
        }
      });
