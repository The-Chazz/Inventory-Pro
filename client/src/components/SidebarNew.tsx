import React, { useState, useEffect } from './../../node_modules/.vite/deps_temp_343513f7/react';
...
const SidebarNew: React.FC = () => {
  ...
  const [user.role, setUserRole] = useState("Administrator");
  ...
  const updateUserInfoFromSession = () => {
    ...
    if (userInfo) {
      try {
        ...
        setUserRole(user.user.role || "Administrator");
      } catch (error) {
  ...
  // Define navigation items directly instead of using hook
  const navItems = [
    { 
      id: 'dashboard', 
      label: 'Dashboard', 
    ...
    // Business Analytics item removed as requested
    ...
    { 
      ...
      icon: 'fa-dollar-sign',
      path: '/profit-tracker',
      isActive: location === '/profit-tracker',
    },
    { 
      id: 'settings', 
  ...
  switch (user.role) {
    case "Administrator":
      return navItems;
    case "Manager":
      return navItems.filter(item => !['users', 'logs'].includes(item.id));
    case "Cashier":
      return navItems.filter(item => ['dashboard', 'pos', 'sales'].includes(item.id));
    case "Stocker":
      return navItems.filter(item => ['dashboard', 'inventory', 'losses'].includes(item.id));
    default:
      // If user.role is missing or unrecognized, show all for safety in dev/prod
      console.warn("Unrecognized user userRole:", user.role);
      return navItems;
  }
})();
      ...
      <div className="p-4 flex justify-between items-center border-b border-blue-700">
        <div className="flex items-center space-x-2">
          ...
          <div className={`sidebar-text ${sidebarCollapsed ? 'hidden' : ''}`}>
            ...
            <div className="text-xs text-blue-200" id="current-user-user.role">{user.role}</div>
          </div>
        </div>
        <button 
          ...
          {filteredNavItems.map((item) => (
            ...
            >
              <span className={`${sidebarCollapsed ? '' : 'mr-3'}`}>
                ...
                {/* Reports icon removed as requested */}
                {item.id === 'losses' && (
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
