...
import { z } from "zod";
import ./../client/node_modules/canvg/node_modules/regenerator-runtime/./../client/node_modules/canvg/node_modules/regenerator-runtime/path from "path";
import { fileURLToPath } from './../client/node_modules/caniuse-lite/data/features/url';
 ...
 * @returns User object with id, username, and userRole
 */
      ...
      try {
        ...
        if (userData && 
          ...
          if (!['Administrator', 'Manager', 'Cashier', 'Stocker', 'system'].includes(userData.userRole)) {
            console.warn(`Invalid user.role detected: ${userData.userRole}, defaulting to Cashier`);
            userData.userRole = 'Cashier'; // Default to lowest privileged userRole
          }
...
/**
 * Middleware to check if user has Administrator userRole
 * 
...
const isAdmin = (req: Request, res: Response, next: NextFunction) => {
  ...
  // Check if user exists and has required userRole
...
/**
 * Middleware to check if user has Administrator or Manager userRole
 * 
...
const isAdminOrManager = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (currentUser.userRole !== 'Administrator' && currentUser.userRole !== 'Manager') {
    // Log unauthorized access attempt for security monitoring
    console.warn(`Unauthorized management access attempt by ${currentUser.username} (ID: ${currentUser.id}, Role: ${currentUser.userRole})`);
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.put("/api/inventory/:id", async (req: Request, res: Response) => {
    try {
      ...
      // Prevent Stocker userRole from changing item prices
  ...
  app.delete("/api/inventory/:id", async (req: Request, res: Response) => {
    try {
      ...
      // Prevent Stocker userRole from deleting inventory items
